    ...
    for classi in range(y_true.shape[1]):
        ...
        for i in range(len(y_predi)):
            ...
            if y_truei[i] == y_predi[i] == 1:
                TP += 1.0 / sample_weight
            if (y_predi[i] == 1) and (y_truei[i] != y_predi[i]):
                FP += 1.0 / sample_weight
            if y_truei[i] == y_predi[i] == 0:
                TN += 1.0 / sample_weight
            if (y_predi[i] == 0) and (y_truei[i] != y_predi[i]):
                FN += 1.0 / sample_weight
...
def find_optimal_cutoff_thresholds(y_true, y_pred):
    return [
        find_optimal_cutoff_threshold(y_true[:, i], y_pred[:, i])
...
def find_optimal_cutoff_thresholds_for_Gbeta(y_true, y_pred):
    ...
    return [
        find_optimal_cutoff_threshold_for_Gbeta(
            y_true[:, k][:, np.newaxis], y_pred[:, k][:, np.newaxis]
        )
...
def generate_ptbxl_summary_table(selection=None, folder="../output/"):
    ...
    for i, exp in enumerate(exps):
        if selection is None:
            exp_models = [
                m.split("/")[-1] for m in glob.glob(folder + str(exp) + "/models/*")
            ]
        else:
    ...
    for m in models:
        ...
        for e in exps:
            ...
            try:
                me_res = pd.read_csv(
                    folder + str(e) + "/models/" + str(m) + "/results/te_results.csv",
                    index_col=0,
                )

                mean1 = me_res.loc["point"][metric1]
                unc1 = max(
                    me_res.loc["upper"][metric1] - me_res.loc["point"][metric1],
                    me_res.loc["point"][metric1] - me_res.loc["lower"][metric1],
                )

                results_dic[e + "_AUC"].append(
                    "%.3f(%.2d)" % (np.round(mean1, 3), int(unc1 * 1000))
                )
    ...
    for i, e in enumerate(exps):
        ...
        md_source += "|---:|:---|:---|:---| \n"
        for row in (
            df_rest[["Method", e + "_AUC"]]
            .sort_values(e + "_AUC", ascending=False)
            .values
        ):
            md_source += (
                "| "
                + row[0].replace("fastai_", "")
                + " | "
                + row[1]
                + " | [our work]("
                + our_work
...
def ICBEBE_table(selection=None, folder="../output/"):
    ...
    if selection is None:
        models = [
            m.split("/")[-1].split("_pretrained")[0]
            for m in glob.glob(folder + "exp_ICBEB/models/*")
        ]
    else:
        models = []
        for s in selection:
    ...
    for model in models:
        me_res = pd.read_csv(
            folder + "exp_ICBEB/models/" + model + "/results/te_results.csv",
            index_col=0,
        )
        mcol = []
        for col in cols:
            ...
            unc = max(
                me_res.ix["upper"][col] - me_res.ix["point"][col],
    ...
    for i, row in enumerate(df_rest[cols].values):
        md_source += (
            "| "
            + df_rest.index[i].replace("fastai_", "")
            + " | "
            + row[0]
            + " | "
            + row[1]
