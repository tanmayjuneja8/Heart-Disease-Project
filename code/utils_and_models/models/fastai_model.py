...
class metric_func(Callback):
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        if self.metric_component is not None:
            return add_metrics(
                last_metrics, self.metric_complete[self.metric_component]
            )
        else:
...
class fastai_model(ClassificationModel):
    def __init__(
        ...
        self.pretrainedfolder = (
            None if pretrainedfolder is None else Path(pretrainedfolder)
        )
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        if self.pretrainedfolder is None:  # from scratch
            ...
            output_layer_new = nn.Linear(
                output_layer.in_features, self.num_classes
            ).cuda()
    ...
    def _get_learner(self, X_train, y_train, X_val, y_val, num_classes=None):
        ...
        # passing the default kernel size of 5 leads to a max kernel size of 40-1 in the inception model as proposed in the original paper
            ...
            self.name == "fastai_inception1d_no_residual"
        ...
        if self.early_stopping is not None:
            ...
            if (
                self.early_stopping == "macro_auc"
                and self.loss != "mse"
                and self.loss != "nll_regression"
            ):
                metric = metric_func(
                    auc_metric,
                    self.early_stopping,
