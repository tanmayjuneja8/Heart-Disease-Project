...
class SCP_Experiment:
    ...
    def perform(self):
        ...
        for model_description in os.listdir(
            self.outputfolder + self.experiment_name + "/models/"
        ):
    ...
    def evaluate(
        ...
        y_test = np.load(
            self.outputfolder + self.experiment_name + "/data/y_test.npy",
            allow_pickle=True,
        )
        ...
        if bootstrap_eval:
            if not dumped_bootstraps:
                ...
                test_samples = np.array(
                    utils.get_appropriate_bootstrap_samples(
        ...
        test_samples.dump(
            self.outputfolder + self.experiment_name + "/test_bootstrap_ids.npy"
        )
        # val_samples.dump(self.outputfolder+self.experiment_name+'/val_bootstrap_ids.npy')

        # iterate over all models fitted so far
        for m in sorted(
            os.listdir(self.outputfolder + self.experiment_name + "/models")
        ):
            print(m)
            mpath = self.outputfolder + self.experiment_name + "/models/" + m + "/"
            rpath = (
                self.outputfolder + self.experiment_name + "/models/" + m + "/results/"
            )
